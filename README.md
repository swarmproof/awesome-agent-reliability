...
**Tags:** `maintained` (runnable, actively developed) · `research` (paper / dataset / spec) · `commercial` (closed, paid). See [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md) for the tagging rubric and inclusion criteria.
...
- [Why Do Multi-Agent LLM Systems Fail? (MAST)](https://arxiv.org/abs/2503.13657) — NeurIPS 2025; 14 empirical failure modes across 3 categories from 200+ traces / 7 frameworks. Ships a [dataset](https://huggingface.co/datasets/mcemri/MAST-Data) and [code](https://github.com/multi-agent-systems-failure-taxonomy/MAST). `research`
...
- [DBOS](https://github.com/dbos-inc/dbos-transact-py) — durable execution backed by Postgres; inherently durable without explicit checkpoint instrumentation. `maintained`
...
- [τ-bench](https://github.com/sierra-research/tau-bench) / [τ²-bench](https://github.com/sierra-research/tau2-bench) — policy adherence + `pass^k` consistency; the most reliability-relevant benchmark (a task completed but policy-violating = fail). `research`
...
- [OWASP Top 10 for LLM Applications](https://genai.owasp.org) — model-layer risks (prompt injection, data poisoning). `research`
...
PRs welcome — see [`CONTRIBUTING.md`](./CONTRIBUTING.md). Quality bar (full criteria in [`docs/ARCHITECTURE.md`](./docs/ARCHITECTURE.md)): **real · reachable · relevant · non-redundant · honestly described.** One line per entry: `[name](url) — honest one-line description. \`tag\``. Listicles and marketing pages are rejected. Refreshed monthly.
